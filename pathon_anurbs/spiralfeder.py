...
# beam_a.add_node_load_moment(t=beam_a.t1(), force=[0, 0, 0], moment=[0, 1, 1])
...
for step, lam in enumerate(np.linspace(0, 1, 2)[1:]):
    beam_a.make_header(step)

    print(' ##############################')
    print('Force lambda-z: ', lam )  
    M_lam = (2*np.pi*youngs_modulus*Iy/15) * lam /np.sqrt(2)
    print('Load = ',  M_lam) 
