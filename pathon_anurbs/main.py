...
beam_a.add_support(t = beam_a.get_domain_T0(), penalty = penalty_dict, material = 'material_1')
...
beam_a.add_support(t = beam_a.get_domain_T1(), penalty = penalty_dict, material = 'material_1')
...
for step, lam in enumerate(np.linspace(0, 1, 10)[1:]):
    beam_a.set_node_load(index=-1, load=[-0.001*lam, 0 , -0.001*lam  ])
    model.solve()
