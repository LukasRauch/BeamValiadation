...
beam_b.fix_node(index=6, directions=['x', 'y', 'z', 'rotation'])
beam_b.fix_node(index=5, directions=['y', 'z'])
...
beam_a.add_support(t = beam_a.t0(), penalty = penalty_dict)
...
# beam_a.add_support(t = beam_a.t1(), penalty = penalty_dict)
...
beam_b.add_node_load(index=0)

for step, lam in enumerate(np.linspace(0, 1, 11)[1:]):
    print('Force lambda-z: ', lam )
    # beam_a.set_node_load(index=-1, load=[-0.001*lam, 0 , 0])
    # beam_a.set_node_load(index=-1, load=[0,  -0.001*lam ,0])
    beam_a.set_node_load(index=-1, load=[0, 0 , -50*lam])
    # beam_a.set_node_load(index=-1, load=[0, 0 , 0 ])

    beam_b.set_node_load(index=0, load=[0, 100*lam, 0 ])
